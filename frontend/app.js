/* ══════════════════════════════════════
   ...
   Home profile fixed: always shows owner/admin profile
...
function show(id) {
  ["homeView", "chatView", "adminView"].forEach(x => {
    const el = $(x);
...
/* ══ Load Current User + Owner Profile ══ */
...
async function loadMe() {
  ...
  try {
    /*
      ...
      state.me = CURRENT LOGGED-IN TELEGRAM USER.

      It is used ONLY for:
      - authentication
      - subscription
      - admin access
      - user's own profile editing

      The HOME screen must NOT display state.me's
      name/username/avatar.

      HOME screen always displays OWNER_PROFILE.
    */
    ...
    // Owner/Admin identity shown to every visitor.
    const owner = {
      ...OWNER_PROFILE,
      ...(state.me.admin_profile || {})
    };

    const ownerName =
      owner.first_name || "Sanya Chouhan";

    const ownerUsername =
      owner.username || "sanyachouhaan_bot";

    const ownerBio =
      owner.bio || "Private messaging inside Telegram.";
    ...
    $("brandName").textContent = ownerName;
    $("statusText").textContent = "Messenger";
    ...
    $("chatHeadName").textContent = ownerName;
    ...
    /*
      Admin does NOT need to purchase Premium.

      Normal user:
      - active Premium => hide subscribe
      - free => show 199 Stars button
    */
...
async function openChat() {
  ...
  /*
    ...
    Subscription requirement remains.

    Admin = allowed without Premium.
    Normal user = active Premium required.
  */
  ...
  /*
    ...
    The home profile shown above remains Sanya.
  */
...
function renderMessages(
  ...
  list.forEach(m => {
    ...
    /*
      sender_id is the actual Telegram user who sent message.
      ...
      Therefore we NEVER compare against
      the visitor/user profile identity incorrectly.
    */
    ...
    const tick =
      isMine
        ? `<span class="tick ${m.seen_at ? "seen" : ""}">✓✓</span>`
        : "";
...
function showMsgCtx(
  ...
  ctx.innerHTML =
    items.map(item => `
      <div
        class="msg-ctx-item ${item.danger ? "danger" : ""}"
...
async function subscribe() {
  ...
  try {
    ...
    /*
      Backend invoice = 199 Telegram Stars.
      Use POST because backend expects the
...
function openProfileEdit() {
  ...
  /*
    Edit Profile is the CURRENT USER profile.

    This does NOT change the Sanya owner profile
...
async function openAdminChat(
  ...
  if (userData?.avatar_url) {
    ...
    avatarEl.innerHTML = `
      <img
        src="${escapeHtml(userData.avatar_url)}"
        alt=""
        style="
          width:100%;
          height:100%;
